...
export default function SeekerSidebar() {
  ...
  // Count active applied applications
  const appliedCount = jobs.filter((j) => j.status === 'Applied' || j.status === 'Under Review' || j.status === 'Interviewing').length;

  const links = [
    { href: '/dashboard/seeker', label: 'Overview', icon: 'dashboard' },
    { href: '/dashboard/seeker/applications', label: 'My Applications', icon: 'list_alt', count: appliedCount },
    { href: '/dashboard/seeker/profile', label: 'My Profile', icon: 'person' },
    { href: '/dashboard/seeker/interviews', label: 'Assessments & Prep', icon: 'quiz' },
  ...
  return (
    <aside className="fixed left-0 top-0 h-full w-[260px] bg-[#081126] border-r border-outline-variant/15 flex flex-col py-lg z-50">
      <div className="px-lg mb-xl">
        ...
        <p className="font-body-sm text-body-sm text-outline-variant/70 mt-2">Professional Portal</p>
      </div>
      ...
      <div className="border-t border-white/10 pt-md space-y-xs text-[13px]">
        ...
        >
          ...
          <span className="font-title-sm text-title-sm">Help Desk</span>
        </Link>
        <button
          onClick={() => router.push('/')}
